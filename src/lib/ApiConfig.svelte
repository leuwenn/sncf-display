<script lang="ts">
	...
	export let stationId = import.meta.env.VITE_DEFAULT_STATION_ID || '87286005';
	export let stationName = import.meta.env.VITE_DEFAULT_STATION_NAME || 'Lille Flandres';
	...
	async function handleSearch() {
		if (!apiKey) {
			errorMessage = 'Aucune clé API n\'est configurée dans le fichier .env';
			return;
		}
		...
		try {
			...
			errorMessage = 'Erreur lors de la recherche: vérifiez votre clé API dans le fichier .env';
			console.error(error);
		} finally {
			searching = false;
		}
...
<div class="api-config">
	...
	<div class="form-group">
		<label>Gare sélectionnée: <strong>{stationName}</strong></label>
				...
				<ul>
					{#each searchResults as result}
						{#if result.type === 'stop_area'}
							<li on:click={() => selectStation(result)}>
								{result.name}
							</li>
						{/if}
					{/each}
