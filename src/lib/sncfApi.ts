...
export async function fetchDepartures(
  ...
  try {
    ...
    const now = new Date();
    const date = new Date(now.getTime() - 30 * 60000); // Départ depuis 30 minutes
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Erreur API SNCF: ${response.status} ${response.statusText} - ${errorText}`,
      );
    }
...
function formatDepartures(apiData: any): SncfDeparture[] {
  ...
  return apiData.departures.map((departure: any) => {
    ...
    if (commercialMode.includes("TGV") || headsign.includes("TGV")) {
      ...
      trainNumber = tripShortName
        ? `INTERCITÉS ${tripShortName}`
        : "INTERCITÉS";
    ...
    if (
      ...
      stopTime.data_freshness === "canceled"
    ) {
      status = "cancelled";
    ...
    if (!platform && departure.stop_point?.name) {
      const match = departure.stop_point.name.match(
        /(?:voie|quai)\s+(\d+[A-Za-z]?)/i,
      );
      if (match && match[1]) {
        platform = match[1];
      }
    ...
    return {
      id:
        departure.id ||
        `departure-${Math.random().toString(36).substring(2, 9)}`,
...
function getDisruptionReason(departure: any, disruptions: any[]): string {
  ...
  if (departure.disruption_id && disruptions) {
    const disruption = disruptions.find(
      (d: any) => d.id === departure.disruption_id,
    );
    if (disruption) {
      return (
        disruption.cause ||
...
export async function searchStation(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Erreur API SNCF: ${response.status} ${response.statusText}`,
      );
    }
